...
// ==========================================
// 定义所有可配置项的默认值，确保插件安装后立即可用
function sgn_defaults() {
    return array(
        'model_type' => 'cat',          // 默认形象：黑猫
        'mobile_display' => '0',        // 移动端默认不隐藏
        'api_url' => 'https://v1.hitokoto.cn/?c=b', // 默认一言 API

        // 功能开关 (1=开启, 0=关闭)
        'enable_drag' => '1',           // 拖拽重力
        'enable_sleep' => '1',          // 睡眠系统
        'enable_scroll' => '1',         // 滚动失重
        'enable_visibility' => '1',     // 离屏互动
        'enable_reading' => '1',        // 鼠标伴读
        'enable_toy' => '1',            // 投掷喂食
        'enable_attack' => '1',         // 攻击特效
        'sleep_start' => '23',          // 入睡时间 (24小时制)
        'sleep_end' => '6',             // 醒来时间

        // 行为概率 (0-100)
        'action_chance' => '30',        // 随机动作概率
        'talk_chance' => '45',          // 说话概率
        'reading_chance' => '30',       // 伴读触发概率
        'toy_chance' => '65',           // 喂食成功概率
        'attack_chance' => '5',         // 攻击触发概率
        'idle_sleep_ms' => '300000',    // 闲置多久入睡 (毫秒)
...
function sgn_resolve_model($opt) {
    ...
    if ($resolved !== null) return $resolved; // 缓存结果，避免一次请求变多次
...
class SGN_Settings {
    ...
    public function create_admin_page() {
        ...
        ?>
        <style>
            /* 强制隐藏 WordPress 原生丑陋的提示框 */
            div.updated, div.error, div.notice { display: none !important; }

            /* CSS 变量定义 */
            :root {
                --sgn-bg: #f5f5f7;
                --sgn-sidebar: rgba(255, 255, 255, 0.75);
                --sgn-card: #ffffff;
                --sgn-accent: #007aff;
                --sgn-text: #1d1d1f;
                --sgn-border: #d2d2d7;
            ...
            .sgn-logo-area h1 { font-size: 20px; font-weight: 700; margin: 0; color: #000; }
            ...
            .sgn-nav-item {
                ...
                cursor: pointer; transition: 0.2s; font-weight: 500; font-size: 13px; color: #444;
            }
            .sgn-nav-item:hover { background: rgba(0,0,0,0.04); }
            .sgn-nav-item.active { background: var(--sgn-accent); color: #fff; box-shadow: 0 2px 8px rgba(0,122,255,0.2); }
            ...
            .sgn-group-title {
                font-size: 13px; font-weight: 600; text-transform: uppercase; color: #86868b;
                margin-bottom: 20px; letter-spacing: 0.5px; border-bottom: 1px solid #f0f0f0; padding-bottom: 8px;
            }

            /* 表单元素优化 */
            .form-table th { padding: 20px 10px 20px 0; width: 160px; font-weight: 500; vertical-align: top; }
            .sgn-input, .sgn-textarea {
                width: 100%; max-width: 450px; padding: 10px 12px; border: 1px solid var(--sgn-border);
                border-radius: 8px; font-size: 14px; background: #fff; transition: 0.2s;
            }
            ...
            /* iOS 风格开关 */
            .sgn-switch { position: relative; display: inline-block; width: 42px; height: 24px; vertical-align: middle; margin-right: 10px; }
            ...
            input:checked + .sgn-slider:before { transform: translateX(18px); }

            .sgn-footer { margin-top: 50px; padding-top: 20px; border-top: 1px solid #f0f0f0; text-align: right; }
            .button-primary.sgn-btn {
                background: var(--sgn-accent) !important; border: none !important; border-radius: 8px !important;
                padding: 10px 30px !important; font-size: 14px !important; font-weight: 600 !important; box-shadow: 0 4px 12px rgba(0,122,255,0.3) !important;
                transition: transform 0.1s; height: auto !important; line-height: normal !important;
            }
            .button-primary.sgn-btn:active { transform: scale(0.98); }
            ...
            #sgn-toast {
                ...
                color: #333; padding: 10px 24px; border-radius: 30px;
                box-shadow: 0 8px 30px rgba(0,0,0,0.12);
                display: flex; align-items: center; gap: 8px;
                font-weight: 500; font-size: 13px; z-index: 999999;
            ...
            <form method="post" action="options.php">
                ...
                <div class="sgn-layout">
                    <div class="sgn-sidebar">
                        <div class="sgn-logo-area">
                            <h1>Neko <span class="version">v<?php echo SGN_VERSION; ?></span></h1>
                        </div>
                        <div class="sgn-nav-item active" data-tab="tab-main">
                            <span class="sgn-nav-icon">🎨</span> 外观
                        </div>
                        <div class="sgn-nav-item" data-tab="tab-features">
                            <span class="sgn-nav-icon">⚡️</span> 功能
                        </div>
                        <div class="sgn-nav-item" data-tab="tab-tune">
                            <span class="sgn-nav-icon">🎛️</span> 调参
                        </div>
                        <div class="sgn-nav-item" data-tab="tab-quotes">
                            <span class="sgn-nav-icon">💬</span> 语录
                        </div>
                        <div class="sgn-nav-item" data-tab="tab-about">
                            <span class="sgn-nav-icon">ℹ️</span> 关于
                        </div>
                    </div>
                    ...
                    <div class="sgn-content">
                        <div id="tab-main" class="sgn-tab-pane active">
                            ...
                            <table class="form-table" role="presentation">
                                <?php do_settings_fields('shiguang-neko', 'sgn_main'); ?>
                            </table>
                        </div>
                        ...
                        <div id="tab-features" class="sgn-tab-pane">
                            ...
                            <table class="form-table" role="presentation">
                                <?php do_settings_fields('shiguang-neko', 'sgn_features'); ?>
                            </table>
                        </div>
                        ...
                        <div id="tab-tune" class="sgn-tab-pane">
                            ...
                            <table class="form-table" role="presentation">
                                <?php do_settings_fields('shiguang-neko', 'sgn_tune'); ?>
                            </table>
                        </div>
                        ...
                        <div id="tab-quotes" class="sgn-tab-pane">
                            ...
                            <table class="form-table" role="presentation">
                                <?php do_settings_fields('shiguang-neko', 'sgn_quotes'); ?>
                            </table>
                        </div>

                        <div id="tab-about" class="sgn-tab-pane">
                            <div style="padding: 20px; background: #f9f9f9; border-radius: 12px; color: #666; text-align:center;">
                                <h2 style="margin-top:0; color:#333;">👾 Shiguang Neko</h2>
                                <p>原生 WordPress 像素宠物插件，带给您最纯粹的桌面陪伴。</p>
                                <p><strong>Version: <?php echo SGN_VERSION; ?></strong></p>
                                <p>Made with ❤️ by Shiguang</p>
                            </div>
                        </div>

                        <div class="sgn-footer">
                            <?php submit_button('保存所有设置', 'primary sgn-btn'); ?>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <script>
            jQuery(document).ready(function($){
                ...
                $('.sgn-nav-item').click(function(){
                    $('.sgn-nav-item').removeClass('active');
                    $(this).addClass('active');
                    var target = $(this).data('tab');
                    $('.sgn-tab-pane').hide().removeClass('active');
                    $('#' + target).fadeIn(200).addClass('active');
                });

                // Toast 显示逻辑 (仅在保存成功后触发)
                <?php if ($is_saved) : ?>
                setTimeout(function(){
                    $('#sgn-toast').addClass('show');
                    setTimeout(function(){ $('#sgn-toast').removeClass('show'); }, 3000);
                }, 500);
                <?php endif; ?>
            });
        </script>
    ...
    private function textarea($key) {
        echo "<textarea name='sgn_options[" . esc_attr($key) . "]' class='sgn-textarea' rows='5'>" . esc_textarea($this->opt($key)) . "</textarea>";
    }
    ...
    public function field_model_type() {
        ...
        foreach ($models as $val => $label) {
            echo '<label style="margin-right: 15px; cursor:pointer;"><input type="radio" name="sgn_options[model_type]" value="' . $val . '" ' . checked($val, $cur, false) . '> ' . $label . '</label>';
        }
    }
